...
function getWeatherData() {
    navigator.geolocation.getCurrentPosition((success) => {
        ...
        fetch(`https://api.weatherapi.com/v1/current.json?key=${API_KEY}&q=${latitude},${longitude}&exclude=hourly,minutely`).then(res => res.json()).then(data => {
            console.log(data);
            // showWeatherData(data);

            fetch(`https://api.weatherapi.com/v1/forecast.json?key=${API_KEY}&q=${latitude},${longitude}&dt=${date}&days=7`).then(res => res.json()).then(data1 => {
                console.log(data1);
